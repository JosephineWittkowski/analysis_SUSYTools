...
//     if(sumwMapIter != m_sumwMap.end()) recalc_sumw = sumwMapIter->second;
  ...
  clearTruthObjects();
  selectTruthObjects();
  TruthParticleVector truthElectrons = getPreTruthLeptons(&nt,11);
  TruthParticleVector truthMuons     = getPreTruthLeptons(&nt,13);
  TruthParticleVector truthTaus      = getPreTruthLeptons(&nt,15);
  TruthJetVector truthJets      = getPreTruthJets   (&nt   );
  ...
  TruthParticleVector signal_truthElectrons ;
  TruthParticleVector signal_truthMuons     ;
  TruthParticleVector signal_truthTaus      ;
  TruthJetVector signal_truthJets;
  
  getSignalTruthObjects  (truthElectrons, truthMuons, truthTaus, truthJets,
                          signal_truthElectrons, signal_truthMuons, signal_truthTaus, signal_truthJets);
...
//   //event cleaning doesn't need to be changed after agreement on cutflow:
  if(doEventCleaning_andFillHistos(truthJets, m_truthMet, truthMuons, truthElectrons, flag, weight_ALL_EE, weight_ALL_MM, weight_ALL_EM, SysSetting, n0150BugFix)){
...
// 	      TruthParticleVector electrons_SS;
	      ...
	      el0_SS = (truthElectrons.at(0)->Pt() > truthElectrons.at(1)->Pt()) ? truthElectrons.at(0) : truthElectrons.at(1);
	      el1_SS = (truthElectrons.at(0)->Pt() > truthElectrons.at(1)->Pt()) ? truthElectrons.at(1) : truthElectrons.at(0);
	      TLorentzVector el0_SS_TLV(*el0_SS);
	      TLorentzVector el1_SS_TLV(*el1_SS);
	      TLorentzVector truthMet_TLV = m_truthMet->lv();
...
// 		  cout << "el0_SS->Pt()= " << el0_SS->Pt() << " el1_SS->Pt()= " << el1_SS->Pt() << endl;
		  if(el0_SS->Pt() > 30. && el1_SS->Pt() > 20.){
		    cutnumber = 26.;  fillHistos_EE_SRSS1(cutnumber, weight_ALL_EE); //ZVeto
		  ...
		  if(el0_SS->Pt() > 30. && el1_SS->Pt() > 20.){
			...
			if(Mljj_EE < 120.){
			  ...
			  if(METrel_EE>=30.){			    
			    ...
			    if(calcSysUncert) cutflow_EE_sysUncert->Fill(cutnumber, isys, weight_ALL_EE);
			  }
			}
		      }
		    }
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if(signal_truthTaus.size()==0){
	  cutnumber = 17.; fillHistos_MM_SRSS1(cutnumber, weight_ALL_MM);
		...
		mu0 = (truthMuons.at(0)->Pt() > truthMuons.at(1)->Pt()) ? truthMuons.at(0) : truthMuons.at(1);
		mu1 = (truthMuons.at(0)->Pt() > truthMuons.at(1)->Pt()) ? truthMuons.at(1) : truthMuons.at(0);
			...
			if(mTmax_EM > 110){
			  ...
			  if(Mlj_EM < 90.){
			    ...
			    if(mTWW_EM > 120.){
			      ...
			      if(calcSysUncert) cutflow_EM_sysUncert->Fill(cutnumber, isys, weight_ALL_EM);
			    }				 
			  }
			}
		      }
	    ...
	    //------------------------------------------------------------------------------------
		  if(nSignalJets >=2 && nSignalJets <=3){
			...
			if(Mljj_EM < 120.){
			  ...
			  if(mTWW_EM >= 110.){
			    cutnumber = 40.; fillHistos_EM_SRSS1(cutnumber, weight_ALL_EM);
			    if(calcSysUncert)cutflow_EM_sysUncert->Fill(cutnumber, isys, weight_ALL_EM);
			  }
			}
		      }
...
void TSelector_SusyNtuple_Truth::SlaveTerminate()
{
  ...
  if(m_writeOut) {
    ...
    if(sample_identifier == 157814)outputfile="histos_ZN_WZ_n0150_Truth_";
